...
class WBV_REST_Controller {
            ...
            foreach ( $product_query->posts as $pid ) {
                ...
                foreach ( $filtered_variation_ids as $vid ) {
                    ...
                    $variation_items[] = array(
                        ...
                        'attributes'    => array_map( function( $a ) { return array( 'key' => sanitize_text_field( $a['key'] ), 'label' => sanitize_text_field( $a['label'] ), 'value' => sanitize_text_field( $a['value'] ) ); }, $attrs ),
    ...
    public static function handle_undo( WP_REST_Request $request ) {
        ...
        if ( $use_async ) {
            $variation_ids = array_map( function ( $r ) { return (int) $r->variation_id; }, $orig_rows );
